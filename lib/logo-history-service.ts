...
import { getUserInvoices } from './invoice-service-server'
...
export const getUserLogoHistory = async (): Promise<UsedLogo[]> => {
	try {
		...
		const usedLogos: UsedLogo[] = Array.from(logoMap.entries()).map(
			([url, data]) => ({
				url,
